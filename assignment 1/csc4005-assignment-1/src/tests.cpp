...
TEST(OddEvenSort, 10k) {
    int rank;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    if (rank == 0) {
        std::vector<Element> data(10'000);
...
TEST(OddEvenSort, 20k) {
    int rank;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    if (rank == 0) {
        std::vector<Element> data(20'000);
        auto dev = std::random_device{};
        auto seed = dev();
        auto gen = std::default_random_engine(seed);
        auto dist = std::uniform_int_distribution<Element>{};
...
TEST(OddEvenSort, 40k) {
    int rank;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    if (rank == 0) {
        std::vector<Element> data(40'000);
        auto dev = std::random_device{};
        auto seed = dev();
        auto gen = std::default_random_engine(seed);
        auto dist = std::uniform_int_distribution<Element>{};
...
TEST(OddEvenSort, 80k) {
    int rank;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    if (rank == 0) {
        std::vector<Element> data(80'000);
        auto dev = std::random_device{};
        auto seed = dev();
        auto gen = std::default_random_engine(seed);
        auto dist = std::uniform_int_distribution<Element>{};
...
TEST(OddEvenSort, 160k) {
    int rank;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    if (rank == 0) {
        std::vector<Element> data(160'000);
        auto dev = std::random_device{};
        auto seed = dev();
        auto gen = std::default_random_engine(seed);
        auto dist = std::uniform_int_distribution<Element>{};
...
TEST(OddEvenSort, 320k) {
    int rank;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    if (rank == 0) {
        std::vector<Element> data(320'000);
        auto dev = std::random_device{};
        auto seed = dev();
        auto gen = std::default_random_engine(seed);
        auto dist = std::uniform_int_distribution<Element>{};
