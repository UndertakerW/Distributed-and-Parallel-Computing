...
namespace hdist {
    ...
    struct Grid {
        ...
        explicit Grid(size_t size,
                      ...
                      size_t x,
                      size_t y)
                : data0(size * capacity), data1(size * capacity), length(size) {
            for (size_t i = 0; i < length; ++i) {
                for (size_t j = 0; j < length; ++j) {
                    if (i == 0 || j == 0 || i == length - 1 || j == length - 1) {
                        this->operator[]({i, j}) = border_temp;
        ...
        std::vector<double> &get_alt_buffer() {
    ...
    bool calculate_jacobi(const State &state, Grid &grid, int tid, int thread_chunk, int rank, int proc_chunk, int n_threads) {

        bool stabilized = true;

        int i_max = 0;
        if (rank * proc_chunk + (tid + 1) * thread_chunk < (rank + 1) * proc_chunk) {
            i_max = rank * proc_chunk + (tid + 1) * thread_chunk;
        }
        else if (rank * proc_chunk + (tid + 1) * thread_chunk < state.room_size) {
            i_max = (rank + 1) * proc_chunk;
        }
        else {
            i_max = state.room_size;
        }

        for (size_t i = rank * proc_chunk + tid * thread_chunk; i < i_max; ++i) {
            for (size_t j = 0; j < state.room_size; ++j) {
                auto result = update_single(i, j, grid, state);
                stabilized &= result.stable;
                grid[{alt, i, j}] = result.temp;
    ...
    bool calculate_sor(const State &state, Grid &grid, int tid, int thread_chunk, int rank, int proc_chunk, int n_threads, int k) {
